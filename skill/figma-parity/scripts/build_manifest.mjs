...
const requested = [...compared, ...declaredMissing.map(({ reason, ...c }) => c)];
...
const manifest = {
  ...
  coverage: { requested, compared, missing: declaredMissing },
  evidence,
...
process.stdout.write(`${outPath}\n${evidence.length} evidence item(s), ${compared.length}/${requested.length} cells compared, ${limitations.length} limitation(s)\nNow run: node validate_manifest.mjs ${path.relative(process.cwd(), outPath)}\n`);
